...
const addPurchases = (arr) => {
  // Solution code here...
  let result= arr.reduce((prev,curr)=>{return prev+=curr.purchasePrice},0);
  return result;
};
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let result= arr.reduce(val=>val+1,0);
  return result;
};
...
Write a function named returnNames that, given the Star Wars data, below, uses reduce to return an array containing 
...
const reversedString = (str) => {
  // Solution code here...
  let split=str.split('');
  let result=split.reduce((acc,val)=>{
    return val+acc;
  })
  return result;
};
